...
contract KryptonaDAO is Ownable {
    ...
    constructor(address _kryptonaTokenAddress, address _treasury) Ownable(msg.sender) {
    ...
    function contributeToTreasuryKryptonaTokens(uint256 _amount) public {
        require(kryptonaToken.transferFrom(msg.sender, address(treasury), _amount), "Token transfer failed");
    }

    function sendTreasuryFundsETH(address payable _to, uint256 _amount) public onlyProposalContractOrOwner {
        treasury.sendETH(_to, _amount);
    }

    function sendTreasuryFundsKryptonaTokens(address payable _to, uint256 _amount) public onlyProposalContractOrOwner {
        treasury.sendKryptonaTokens(_to, _amount);
    }
